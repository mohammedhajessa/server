...
function initializeSocket() {
    ...
    // Listen for file download responses
    socket.on('0xFI', function(data) {
        console.log('File response received:', data);
        
        if (data.type === 'download') {
            showNotification('#2ecc71', `File downloaded: ${data.fileName} (${data.fileSizeFormatted})`);
            // Update downloads page if we're on it
            if (window.location.pathname.includes('/downloads')) {
                setTimeout(() => location.reload(), 1000);
            }
        } else if (data.type === 'list') {
            // File list response - reload the page to show new files
            if (window.location.pathname.includes('/file_manager')) {
                setTimeout(() => location.reload(), 500);
            }
        } else if (data.type === 'error') {
            showNotification('#f03434', `File error: ${data.error}`);
        }
    });
    
    // Listen for other command responses
...
$(document).ready(function() {
    ...
    if (pathParts.length >= 3 && pathParts[1] === 'manage') {
        currentDeviceID = pathParts[2];
    }
});
